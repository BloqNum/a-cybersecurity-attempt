...
void walk_block(struct tpacket_block_desc *pbd){
    int packets = pbd->hdr.bh1.num_pkts;
    struct tpacket3_hdr *ppd = NULL;
...
void parser(struct tpacket3_hdr *ppd){
    ...
    ip = (struct iphdr *) ((uint8_t *) ppd + ppd->tp_net);   
    sour.s_addr = htonl(ip->saddr);
    dest.s_addr = htonl(ip->daddr);
    loss = ppd->tp_len - ppd->tp_snaplen;

    printf(" [IP]: %s -> %s [%s -> %s] [Loss] %u\n", inet_ntoa(sour), inet_ntoa(dest), get_ver(ip->version),get_prot(ip->protocol),loss);
...
static char *get_prot(uint8_t prot){
    switch (prot) {
        ...
        case 145: return "NSH";
        case 146: return "Homa";
        case 147: return "BIT-EMU";
